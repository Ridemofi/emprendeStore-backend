...
public interface EmprendedorRepository extends JpaRepository<Emprendedor, Long> {
    @Query(value = "SELECT e.ID_EMPRE, e.NOMBEMP, e.IMAGENEMPRE, e.CORREO, e.NRO_CEL, e.PASSWORD, e.FECHA_REGISTRO " +
            "FROM EMPRENDEDOR e " +
            "JOIN VENTA v ON e.ID_EMPRE = v.ID_EMPRE " +
            "GROUP BY e.ID_EMPRE, e.NOMBEMP, e.IMAGENEMPRE, e.CORREO, e.NRO_CEL, e.PASSWORD, e.FECHA_REGISTRO " +
